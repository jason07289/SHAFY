 ...
 * private User(){ // 이렇게 생성자를 은닉하고
 * }
 * public static User of(){
 * 		return new User();     // 스테틱으로 생성하게 끔 유도 가능
 *  }
 * 
 */
 ...
 * 
 * 
 * 부가적으로 @EqualsAndHashCode(exclude = "id") 이런식으로 
 * Equal메소드랑 해시코드 메소드도 생성 가능
 * */
@Setter
@Getter
 ...
 *
 *@Entity(name="user")로도 table명 지정가능 CamelCase가 java에서 쓰이고 db는 under_score기 때문에 명시해주는게 좋음
 * 	*/
...
public class User {
	...
	/*이거 헷갈림
	 * GenerationType.??
	 * 1. AUTO : 특정 DB에 맞게 자동 선택 jpa에게 생선전략 위임 (개꿀)
	 * 2. INDENTITY : DB의 identity 컬럼을 이용 -> 숫자값에서만 가능
	 * 3. SEQUENCE : DB의 seqeunce 컬럼을 이용
	 * 4. TABLE : 유일성이 보장된 데이터베이스 테이블을 이용
	 * 
	 * AUTO 이외의 생성전략을 사용할 때 DB에 대한 지식이 좀 더 요구된다고함 
	 * 그래서 아직 이해가 안되는듯.. 
...
//	@Setter(AccessLevel.PROTECTED) 이런식으로 속성별 제한을 걸수도 있는듯...
	...
	/* TemporalType.???
	 * date(년월일), time(시분초), timestamp(년월일 시분초) 세가지 중 고르자
	 * default는 timestamp
	 */
